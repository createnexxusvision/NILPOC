...
contract NILDealEscrow is BaseNIL {
    ...
    function createDeal(
        ...
        NILDeal memory newdeal = NILDeal({
            athlete: athlete,
            sponsor: msg.sender,
            amount: msg.value,
            deadline: deadline,
        ...
        _onReceiveETH(msg.sender, msg.value);
    ...
    function markDelivered(uint256 dealId) external {
        ...
        // Reputation bump
        completedDeals[deal.athlete] += 1;
        completedDeals[deal.sponsor] += 1;
    ...
    function raiseDispute(uint256 dealId) external {
        ...
        emit DealRefunded(dealId, deal.sponsor, amount);
    }
    ...
    function getReputation(
        address user
    ) external view returns (uint256 completed, uint256 disputed) {
